...
bool gLightOn = false;
bool gLightSensorEnabled = false; // Controlled by MQTT topic `home/livingroom/light-sensor`
...
void loop() {
  ...
  if (gLightOn) {
    if (gLightSensorEnabled) {
       ...
       // "The brighter the environment, the dimmer the LED" -> Higher analog = Lower brightness
       ...
       if (millis() - lastDebug > 500) {
         lastDebug = millis();
         Serial.print("[Sensor] Analog(6): "); Serial.print(analogVal);
