...
// Avoiding WebSocketsClient, manual implementation of WS frame RFC 6455 to avoid library bugs.
//
...
void geminiTestLoop() {
  if (gState == GS_DONE || gState == GS_ERROR || gState == GS_IDLE) return;
...
static bool wsConnect() {
  ...
  gClient.print("GET "); gClient.print(path); gClient.println(" HTTP/1.1");
  gClient.print("Host: "); gClient.println(GEMINI_HOST);
  ...
  while (!gClient.available() && millis() - t < 10000) delay(10);
  ...
  if (!status.startsWith("HTTP/1.1 101")) {
    ...
    while (gClient.connected() && millis() - t < 3000) {
      if (!gClient.available()) { delay(1); continue; }
    ...
    if (body.length()) Serial.printf("[GEMINI] Body: %.500s\n", body.c_str());
    return false;
  }
...
static void wsSendText(const String& json) {
  ...
  if (len < 126) {
    ...
    hdr[hLen++] = 0x80 | 127;
    for (int i = 7; i >= 0; i--) hdr[hLen++] = (len >> (8 * i)) & 0xFF;
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// Read payload streaming — SSL data arrives in chunks, cannot rely on available()
// Returns false if timeout or disconnect
// ─────────────────────────────────────────────────────────────────────────────

static bool wsReadPayload(uint64_t payloadLen, uint8_t** outBuf, size_t* outLen) {
...
static void processFrames() {
  while (gClient.connected() && gClient.available() >= 2) {
    ...
    if (payloadLen == 126) {
      unsigned long t = millis();
      while (gClient.available() < 2 && millis() - t < 2000) delay(1);
      payloadLen = ((uint16_t)gClient.read() << 8) | gClient.read();
    } else if (payloadLen == 127) {
      unsigned long t = millis();
      while (gClient.available() < 8 && millis() - t < 2000) delay(1);
      payloadLen = 0;
      for (int i = 0; i < 8; i++) payloadLen = (payloadLen << 8) | gClient.read();
    }
    ...
    if (opcode == 0x08) {
      ...
      if (payloadLen >= 2) {
        unsigned long t = millis();
        while ((uint64_t)gClient.available() < payloadLen && millis() - t < 2000) delay(1);
    ...
    if (opcode == 0x09) {
      unsigned long t = millis();
      while ((uint64_t)gClient.available() < payloadLen && millis() - t < 1000) delay(1);
      uint8_t* pingData = payloadLen > 0 ? (uint8_t*)malloc(payloadLen) : nullptr;
      if (pingData) { for (uint64_t i = 0; i < payloadLen; i++) pingData[i] = gClient.read(); }
      uint32_t mr = esp_random(); uint8_t mk[4]; memcpy(mk, &mr, 4);
      uint8_t pongHdr[6] = { 0x8A, (uint8_t)(0x80 | (uint8_t)payloadLen),
                              mk[0], mk[1], mk[2], mk[3] };
      gClient.write(pongHdr, 6);
      if (pingData) {
        for (uint64_t i = 0; i < payloadLen; i++) gClient.write(pingData[i] ^ mk[i & 3]);
        free(pingData);
      }
      continue;
    }
    ...
    if (opcode != 0x01 && opcode != 0x02 && opcode != 0x00) {
      unsigned long t = millis();
      while ((uint64_t)gClient.available() < payloadLen && millis() - t < 1000) delay(1);
      for (uint64_t i = 0; i < payloadLen && gClient.available(); i++) gClient.read();
      continue;
    }
...
static void i2sInit() {
  i2s_chan_config_t chan_cfg = I2S_CHANNEL_DEFAULT_CONFIG(GEMINI_I2S_PORT, I2S_ROLE_MASTER);
  ...
  if (err != ESP_OK) { Serial.printf("[GEMINI] i2s_init err: 0x%x\n", err); return; }

  err = i2s_channel_enable(gI2sTxHandle);
  if (err != ESP_OK) { Serial.printf("[GEMINI] i2s_enable err: 0x%x\n", err); return; }
